 ...
 */
DefinitionBlock ("", "DSDT", 2, "LENOVO", "TP-N1C  ", 0x00001130)
{
    /*
     * iASL Warning: There were 10 external control methods found during
     * disassembly, but only 5 were resolved (5 unresolved). Additional
    ...
    Scope (\_SB)
    {
        ...
        Device (PCI0)
        {
            ...
            Device (LPC)
            {
                ...
                Device (MOU)
                {
                    Name (_HID, EisaId ("LEN004B"))  // _HID: Hardware ID
                ...
                Device (EC)
                {
                    ...
                    {
                        If (\_SB.PCI0.LPC.EC.HKEY.MHKK (0x03, 0x4000))
                        {
                            \_SB.PCI0.LPC.EC.HKEY.MHKQ (0x130F)
                        }
                    }
                    ...
                    Device (HKEY)
                    {
                        ...
                        Method (MLCS, 1, NotSerialized)
                        {
                            ...
                            If (LNot (And (Local0, 0x80000000)))
                            {
                                ...
                                ElseIf (\_SB.PCI0.LPC.EC.HKEY.MHKK (0x01, 0x00020000))
                                {
                                    \_SB.PCI0.LPC.EC.HKEY.MHKQ (0x1012)
                                }
                            }
                    ...
                    Scope (\_SB.PCI0.LPC.EC)
                    {
                        Method (_Q6A, 0, NotSerialized)  // _Qxx: EC Query
                        {
                            ...
                            ElseIf (\_SB.PCI0.LPC.EC.HKEY.MHKK (0x01, 0x04000000))
                            {
    ...
    Scope (\_SB.PCI0.LPC.EC.HKEY)
    {
        ...
        Method (DYTC, 1, Serialized)
        {
            ...
            If (LAnd (\WNTF, \TATC))
            {
                While (One)
                {
                    ...
                    ElseIf (LEqual (_T_0, 0x0100))
                    {
                        Store (ShiftLeft (0x1001, 0x10), Local1)
                        Or (Local1, 0x01, Local1)
                    }
                    ElseIf (LEqual (_T_0, 0x01FF))
                    {
                        Store (0x00, \VCQL)
                        Store (0x00, \VTIO)
                        Store (0x00, \VMYH)
        ...
        {
            Store (Arg0, \_SB.PCI0.LPC.EC.HKEY.GPTL)
            If (LAnd (ANGN, LNotEqual (\_SB.PCI0.LPC.EC.VPON, 0x00)))
            {
    ...
    Scope (\_SB.PCI0.XHCI.URTH)
    {
        ...
        Method (PS2X, 0, Serialized)
        {
            ...
            {
                Or (\_SB.PCI0.XHCI.USPP, 0x02, \_SB.PCI0.XHCI.USPP)
            }

            If (LAnd (LEqual (HP02, 0x00), LEqual (SP01, 0x00)))
            {
                Or (\_SB.PCI0.XHCI.USPP, 0x04, \_SB.PCI0.XHCI.USPP)
            }
        }
